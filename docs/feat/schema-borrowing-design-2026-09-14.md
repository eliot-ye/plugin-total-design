...
|---|---|---|---|---|---|---|---|---|
| **A1** review 隔离 | 约束性（零阻塞） | `requesting-code-review` +约 35–45 行 / 2400–3000 字节 → 文件 **+20–25%**，17→18 节 | 0（降级全允许，无阻塞） | **+2 到 +5**（按 tier-small / medium / large 的 review 次数；每次 review 派发 fresh-context subagent = 1 轮） | **600–3000**（subagent report × N） | +1–6 秒 | 0 | 每次 review 都读这节（前置约束），但成本是读一次；**tier-small 下隔离收益递减而派发成本仍要付** |
...
|---|---|---|---|---|---|
| **A1** 隔离 | self-review 盲点长期存在，critical 漏检 | 误报 review（无 critical 判为 critical）→ 阻断推进 | A 有界（下次 review 或用户发现）；B 有界（作者修/重派） | 弱不对称：不做代价略高 | **做，按 tier 分层启用**——tier-large 强制 subagent、tier-medium 优先、tier-small 默认自评+标注（见 8.7） |
...
**风险等级重排**：本稿最高风险项从原"三方同改 `td-propose` 6.c"降为"两方同改同一段"——仍是本稿内部最重要的时序约束，比任何单个 skill 的内部冲突都危险（同一段的两项改动如果不串行，agent 会按后落的那一项覆盖先前一项的必填项，造成契约缺失）。
...
|---|---|---|---|---|
| **A1** 隔离 | **净成本**——隔离收益递减（对象是单文件小改动），但 subagent 派发成本仍要付；降级通道会掩盖此点，实际是"付了不隔离的成本" | 中等收益——补"自评"盲点 | **最高收益**——tier-large 的架构 critical 判据（坏的分系统切分 / 循环依赖 / 隐式依赖）在自评下命中率最低 | 净收益随 tier 单调递增；tier-small 净成本 |
...
- v3 排序第 2 位是 A1+A2——**场景分解暴露偏差**：A1 在 tier-small × maintenance 下是净成本，聚合值把它抬到第 2 位掩盖了这个结构（→ 8.4 判定"按 tier 分层启用"）。
...
| 项 | 判定 | 关键依据 |
|---|---|---|
| **A1** 隔离 | **缩窄范围做**——**建议**按 tier 分层设定"是否必须尝试 subagent"的强度：tier-large **必须尝试** subagent（不可跳过派发动作，但派发失败时按 v3 拍板"全部 tier 均允许降级"降级到自评+`reviewed-by: unisolated-self` 标注）；tier-medium **优先尝试**（可用则派发、不可用即降级，与 v3 拍板完全一致）；tier-small **默认自评**+标注（本档隔离收益递减，不强推派发）。**注：v3 拍板的"全部 tier 均允许降级"是"降级通道打通"（不阻塞、不询问用户），本缩窄是"是否必须尝试派发"的强度分级——两者不冲突，前者管失败后能否继续，后者管是否要花派发成本**。**v5 复核收窄：仅 tier-large 强制启用**（tier-medium / small 不引入派发动作，见 9.4；冲突时以第 9 节为准） | 8.6 场景分解：tier-small × maintenance 下是净成本；8.4 弱不对称 |
...
- **A 档加约束**——不新增文件、不新增阻塞、不新增伪决策点，这三条 v3 判定成立。但 v3 的"重量在文本层"表述按 8.2 订正：A1 的 subagent 派发是流程层重量，A 档整体重量分布是"A1 主导（占 60–80% 的调用轮数与生成 token），A2 / A3 / A4 是文本层增量"。
...
- **A1 的 tier 分层阈值**——本稿建议"tier-large 强制 / tier-medium 优先 / tier-small 默认自评"，具体到落笔时如何用规则表述"强制 subagent"与"优先 subagent"的差别，是 A1 定案里的"落笔措辞细节"（见 3.1 节的已拍板定案最后一条），执行时定稿。（**v5 后由 9.4 覆盖：仅 tier-large 启用**，该措辞细节随之简化）
...
| **A1** review 隔离 | **部分**——tier-large 架构 critical 判据自评确实漏检；多数常规 review 靠外部工具/人审兜底 | 中（仅 tier-large） | 工程导向（收窄后） | **条件必要**——仅 tier-large 强制启用 |
...
| + A1 + A2（限 tier-large） | spec-driven 为主 + review 隔离 | 轻度混合，可接受 |
...
| **A1** | 做，**仅 tier-large 强制启用**——tier-medium / small 不引入派发动作（连"优先尝试"也不保留） | **覆盖 8.7**（8.7 为 tier-medium 保留"优先尝试"）；tier-small × maintenance 下是净成本（8.6），tier-medium 净收益不抵派发成本 |
| **A2** | **随 A1 打包保留**（A1 隔离标注 `reviewed-by` 的机械载体），不独立成立 | **覆盖 8.7**（8.7 独立判"做"）；A1 收窄后其三态锚点的独立核验价值进一步降低 |
...
2. **A1 + A2**（一次原子编辑；A1 仅 tier-large 强制启用）
3. **B1**（必要；tier-small 由 `applyRequires` 自动跳过）
4. ~~A4 / B2 / B3~~——**不做**（价值来源不在工程导向内，或与既有判定一致）

### 9.6 一句话结论

**从工程管理 + 编程任务视角：必要集只有 A3 + B1（堵真实工程漏，td 由此成为 spec-driven 完整超集），条件集只有 A1 + A2（限 tier-large，一次原子编辑）；A4 是 PM 仪表盘、B2 是生态对齐 + 框架外维度、B3 是仪式性——引入它们才是"大杂烩"的开始。v5 建议按最小必要集收敛，最终采纳需用户拍板。**
