...
**系统工程主基调第 4 条：开放的复杂巨系统。** todo 池是"想到了先记下来，别急着建 change"的候选层——候选池可以无限多、不占 WIP，活跃 change 才受 `wip-limit` 约束。这是"开放的复杂巨系统不能并行硬解"的工程化入口之一：把"想做的事"（候选层）与"正在做的事"（WIP）分层隔离，想做的事再多也不撑爆并发。
