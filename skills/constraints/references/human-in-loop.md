...
运行模式由 `openspec/.td-state/autonomy.yaml` 的 `mode` 字段承载（文件不存在视为 `human-in-loop`，模式值由调用方 skill 在激活配置层时读入）。**场景识别不变**——上述必停场景的识别判据与强度叠加（第 1–5 类基线 + 表 1 + 表 2）在两种模式下完全一致；本节只定义 autonomous 模式下识别命中后的**分支差异**：不执行上文「触发时 agent 应做的事」的四步等待，改为查本 change `proposal.md` 的「已确认决策清单」节（该清单按 7 类编号与本文件第 1–7 类一一对应）：
...
| 无法对应到第 1–5 类 | 超出已确认范围 → 回退 |
| 第 7 类（audit 触发） | 一律回退，`reason` 标注"audit 触发第 7 类" |

回退的执行（撤销代码改动 + 日志写入）、运行记录格式与"沿用 proposal 清单确认"的记录动作由调用方承载：`td-apply` 步骤 5（分支编排与回退命令）与 `td-apply` 的 `references/autonomy-template.md`（文件模板与读写规则），本节不重复。第 6 类（WIP override）在 autonomous 模式下结构上不可能触发（见同目录 `wip-limit.md` 的「autonomous 模式下不生效」节）。
