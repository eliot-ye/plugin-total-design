...
本节是 `td-apply` 步骤 7.2 的执行层——`td-apply` 步骤 7.2 定义触发条件与 tier 分层强度，本节定义执行语义。在 `td-apply` 流程里，本节在步骤 7.1 change-level 验证通过之后触发；单独触发本 skill 时，若上游已有 change-level 验证产物，可直接进入本节。
