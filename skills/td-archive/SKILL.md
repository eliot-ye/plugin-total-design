...
archive 前的"实际 vs 预期"复盘（步骤 3）是综合集成循环的闭合动作。
...
archive 之前**必须**在 change 里补一节"实际系统工程影响 vs 预期"——对照 proposal 的"系统工程影响评估"节（含"预期行为模型"字段）。**复盘节统一落在 proposal.md**（检索单一落点；tasks.md 不承载复盘，写错落点的复盘按落在 proposal 对待并搬移）。

**复盘深度按 tier 分层**：

| tier | 必填字段 |
|---|---|
| `tier-small` | 实际影响分系统 + 预期行为模型验证（2 字段） |
| `tier-medium` / `tier-large` | 全部 5 字段（下表） |
...
- scenario→test 覆盖账本状态（若 change 产出过账本）：全绿 / 有哪些 `N/A` 条目及理由
