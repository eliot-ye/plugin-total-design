...
archive 之前**必须**在 change 里补一节"实际系统工程影响 vs 预期"——对照 proposal 的"系统工程影响评估"节（含"预期行为模型"字段），记录：
...
archive 是"完成一个 change"的事件,正好对照表 3(system-audit 频率)。表 3 由 td-* 步骤 1 注入会话上下文;若未注入,调 `field-assessment` 注入后再读。
