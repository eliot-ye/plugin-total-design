...
本 skill 是配置入口，实际内容在 `references/` 下十一个文件：四个机制文件（识别流程 / 强度矩阵 / audit 频率 / 子系统独立定 tier）+ 七个流程/变体文件（config context 引导 + 3 profile + 3 tier，承载流程侧重与特殊规则）。下游 skill 按本 skill「如何被引用」节的读取策略（快车道 / 现判路径）进入，判读后只读命中的 profile 与 tier 各一份变体文件（共 2 份）。

## 如何被引用

每个 td-* skill 的"步骤 1"按以下读取策略判读并注入（重判策略权威在 `references/identification-flow.md`「### 4. 缓存判读结果」节）：
