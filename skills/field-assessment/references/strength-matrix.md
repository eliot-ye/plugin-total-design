...
**表 1 第 5 行是叠加在 `human-in-loop` skill 的 5 类通用基线之上的 tier 额外触发条件，不是绝对强度值。** 5 类通用基线在所有 profile × tier 下都生效——这是"必须停"的下限。

- tier-small"—"：无额外 tier 触发条件，仅用 5 类通用基线。
...
| `human-in-loop` | —（无额外 tier 触发条件，仅用 human-in-loop skill 的 5 类通用基线） | + 公共契约变更 | + 总体设计文档审阅 |
...
| `profile-brownfield` | — | + 改老代码前 | + 总体设计文档审阅前 |
