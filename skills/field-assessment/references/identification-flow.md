...
**本步骤读写边界**：快车道只读不写；现判路径的判读本身也只读不写——写缓存的职责始终在「### 4. 缓存判读结果」。文件格式见下方「### profile-tier.yaml 模板」节（仅现判 / 重判路径读）。
...
引用表 1 / 表 2 / 表 3 的强度值时，按下列规则处理：

- **强度值由 td-* skill 的"步骤 1"注入会话上下文**——按入口 `field-assessment` 的「如何被引用」节读取策略完成注入（快车道或现判路径）。
- **若强度未注入**（如跳过步骤 1、或会话上下文被清理），按入口 `field-assessment` 的「如何被引用」节读取策略重新注入后再读。
