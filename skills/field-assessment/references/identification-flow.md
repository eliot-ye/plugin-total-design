...
- **快车道（缓存命中）** — 文件存在且无重判触发（「### 4. 缓存判读结果」的 3 条）→ 取缓存值为 `$_TD_PROFILE` / `$_TD_TIER`，跳过节 2/3/4，强度注入按入口 `field-assessment` 的「如何被引用」节给出的快车道读取集执行（单一事实源，此处不重列）；快车道下本文件除"子系统独立定 tier 时的强度注入"段（缓存含 subsystems 条目时读）外不读其他节。
...
**层次观归位**：tier 判据是平面维度（文件数、团队规模、部署单元），子系统独立定 tier 机制把它扩展为层次维度（主基调第 4 条「层次观」）——判据在每个子系统层次上分别生效，机制见 `subsystem-tiering.md`。
