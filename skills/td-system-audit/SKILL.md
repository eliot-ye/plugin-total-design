...
先做适用性判断，再决定是否询问：候选资格的权威判定见 `todo-pool` 的「落池条目」子流程的「候选资格判定」节——落池条目必须是 backlog 候选（想做、用户认可、但暂不排期的开发工作）。报告里"建议的下一步动作"中不满足候选资格的（如已交付内容的验收/验证手段），**不适用 → 直接跳过，不向用户提出询问**；只对满足资格的条目（非严重问题、暂不立即修复的后续事项）询问："要不要把这些记进 `openspec/todo.md` 待办池？"——落池 = 记为 backlog 候选，等 `/td-propose` 时从池里挑，不占 WIP。

- 有满足资格的条目且用户同意 → 触发 `todo-pool` 的「落池条目」子流程。
- 用户拒绝，或没有满足资格的条目 → 跳过，不强制。
