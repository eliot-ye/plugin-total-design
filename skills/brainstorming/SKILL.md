...
钱学森综合集成方法里，"允许矛盾"是"反复迭代上升到定量认识"的**起点**，不是**终点**（主基调第 3 条）。正确的工作方式是：

- **允许矛盾并存**：复杂巨系统不能简化还原（主基调第 4 条），强行自洽会损失信息。
- **把矛盾作为反复迭代的输入**：每个矛盾点都是一个待解决的定性判断，需要更多数据/模型来上升到定量认识。
- **最终消除矛盾**：通过综合集成循环（brainstorming → propose → apply → archive），矛盾应该在 archive 的"实际 vs 预期"复盘里被消除。如果 archive 时矛盾仍在，触发 `human-in-loop` 让用户拍板。
