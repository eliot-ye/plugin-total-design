...
"允许矛盾"是"反复迭代上升到定量认识"的**起点**，不是**终点**（主基调第 3 条；允许矛盾并存是因复杂巨系统不能简化还原，第 4 条）：矛盾作为迭代输入，最终在 archive 的"实际 vs 预期"复盘中被消除；archive 时矛盾仍在 → 触发 `constraints` 的 `references/human-in-loop.md` 让用户拍板。

"不急着自洽"不等于"永远不自洽"——前者是"信息不足时不要强行闭合"，后者是放弃综合集成。

遇到可逆决策时调 `constraints` 的 `references/delay-decision.md`，遇到矛盾时按本节处理——本 skill 在总体设计部立场允许矛盾并存、不强行闭合 spec（两者同时生效，权威在 delay-decision）。**完成候选方向评估后，立即执行 `constraints` 的 `references/delay-decision.md` 检查**，判定与处理路径见该文件的「示例：brainstorming 场景」节。
