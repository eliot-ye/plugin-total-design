...
强度本身见 `field-assessment` skill 的表 1（5 个 constraint 在 3 个 tier 下的强度）与表 2（3 个 profile 在 human-in-loop 上的场景加成，已合并进 `field-assessment/references/strength-matrix.md`）。

**profile-maintenance 不再加成其余 4 个 constraint**（`wip-limit` / `critical-buffer` / `brooks-law` / `delay-decision`）——强度由 tier 决定。profile 只决定流程侧重（maintenance 侧重轻量 proposal + 生产稳定性），不改变各 tier 的强度。
