...
**对 plugin 的含义：** profile × tier 二维配置是这个原则的工程化体现——按系统规模分层对待。一个 3 文件的脚本和一个 10 万行的单体系统，不能套同一套 constraint 强度。当系统内部有明显的子系统边界时，`field-assessment` 的识别流程允许子系统独立定 tier（见 `field-assessment` 的「识别流程」节）。
