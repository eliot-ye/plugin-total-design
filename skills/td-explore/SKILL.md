...
explore 是总体设计部（主基调 2）在"想"阶段的工作——先在系统全局立场探索，不直接动手；不简化还原问题（主基调 4），允许矛盾并存。explore 是前馈控制准备环节。
...
explore 阶段不落盘 spec，成果以对话形式交付（见步骤 6）。若用户要求把探索结果落盘为 spec 草稿，提示走 `/td-propose`。

**总体设计部工作方式**：

1. **不直接给方案，先问"想达到什么"**——开场是"你想达到什么？现在为什么不行？"，不是"我建议你这样做"。
2. **候选方向探索**——列出候选方向（不凑数量），系统工程影响 / 可逆性评估维度见步骤 5，并给每个方向列优缺点。对每个方向问"如果选这个，系统整体会怎么变？"——这是总体设计部视角的关键问题。决策已闭合、无新信息时不凑方向——正确输出是一句话"没有需要你的决策点，直接继续推进"，而不是为凑格式生成伪选项（对齐 `constraints` 的 `references/human-in-loop.md` 的"不对所有动作都问一下"红线）。`/td-propose` 步骤 3 的"greenfield explore 检查"会读会话历史判定是否已做过充分探索（判据见 `td-propose` 步骤 3）。
3. **允许矛盾，不急着自洽**——用户说矛盾的话（"要快"又"要稳"）时不强行调和，把矛盾记下来让用户看到。"允许矛盾"是反复迭代上升到定量认识的**起点**不是**终点**（主基调第 3 条）：矛盾作为迭代输入，最终在 archive 的"实际 vs 预期"复盘中被消除；archive 时矛盾仍在 → 触发 `constraints` 的 `references/human-in-loop.md`。"不急着自洽"不等于"永远不自洽"——前者是信息不足时不要强行闭合，后者是放弃综合集成。

**产物要求**：每个候选方向标注系统工程影响与可逆性。产物留在会话上下文里（explore 不落盘 spec，见文末 Guardrails）。

**先查仓库再提问**：向用户提出事实类问题（"现在 X 是怎么做的""有没有 Y 配置"）之前，先在仓库里找答案——代码、`openspec/specs/`、`openspec/config.yaml`、README 与配置文件里已有的事实不问用户，自查后直接带入对话（步骤 2 / 步骤 3 读到的现场背景优先复用）。确实需要问时，问题附**基于现场证据的推荐默认值**（"建议走 A，依据是 <代码 / spec 里的信号>，可以吗"），不出裸问题。仓库证据与用户认知冲突时，摆出证据让用户裁决，不默默采信任一方。

**与 `constraints` 的 `references/delay-decision.md` 的连接**：explore 阶段"不落盘 spec"本质上是延迟决策（语义与触发提醒见该文件「触发时机」节的"与 `td-explore` 的连接"条）。**完成候选方向评估后立即执行 delay-decision 检查**，判定与处理路径见该文件的「示例：探索阶段场景」节。这与 `td-propose` 步骤 3 的"greenfield explore 检查"协同——greenfield 项目先 explore 再 propose。
